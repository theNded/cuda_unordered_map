...
void load_factor_bulk_experiment(uint32_t num_keys,
    ...
    // query ratios to be tested against (fraction of queries that actually
    ...
    for (int i_expected_chain = 0; i_expected_chain < num_sample_lf;
        ...
        auto buffer_ptr = key_gen.getSingleBatchPointer(num_keys, num_queries,
                                                        num_existing);
        ...
        for (int iter = 0; iter < num_iter; iter++) {
            ...
            if (verbose) {
                printf("%d: num_element = %u, load_factor = %.2f, query_ratio "
                       "= %.2f, "
        ...
        object.AddMember("build_time_ms",
                         ...
                         doc.GetAllocator());
        object.AddMember("build_rate_mps",
                         rapidjson::Value().SetDouble(double(num_keys) /
                                                      build_time / 1000.0),
                         doc.GetAllocator());
        ...
        object.AddMember("query_ratio",
                         rapidjson::Value().SetDouble(query_ratio),
                         doc.GetAllocator());
        object.AddMember("load_factor",
                         rapidjson::Value().SetDouble(load_factor),
                         doc.GetAllocator());
    ...
    if (h_key) delete[] h_key;
    if (h_value) delete[] h_value;
    if (h_query) delete[] h_query;
...
void singleton_experiment(uint32_t num_keys,
    ...
    if (verbose) {
        printf("num_element = %u, load_factor = %.2f, query_ratio = %.2f, "
               "num_buckets "
               "= %d\nbuild_rate: %.2f M elements/s\nsearch rate = %.2f M "
               "queries/s\n",
...
void build_search_bulk_experiment(uint32_t num_keys_start,
    ...
    for (uint32_t num_keys = num_keys_start; num_keys <= num_keys_end;
        ...
        const uint32_t num_buckets =
                (num_keys + expected_elements_per_bucket - 1) /
                expected_elements_per_bucket;
        ...
        object.AddMember("exp_chain_length",
                         rapidjson::Value().SetDouble(expected_chain),
                         doc.GetAllocator());
        object.AddMember("num_buckets",
                         rapidjson::Value().SetDouble(num_buckets),
                         doc.GetAllocator());
        object_array.PushBack(object, doc.GetAllocator());
    }
    ...
    for (int i_expected_chain = 0; i_expected_chain < num_sample_lf;
        ...
        for (int iter = 0; iter < num_iter; iter++) {
            ...
            for (batch_id = 0; batch_id < num_initial_batches; batch_id++) {
                ...
                init_build_time += hash_table.MixedOperation(
            ...
            for (; batch_id < num_batches; batch_id++) {
                ...
                concurrent_time += hash_table.MixedOperation(
                        key_gen.h_batch_buffer_, h_result, batch_size,
                        batch_id);
            }
        ...
        if (verbose) {
            printf("expected = %.2f, init_load_factor = %.2f, "
                   "final_load_factor = %.2f, "
