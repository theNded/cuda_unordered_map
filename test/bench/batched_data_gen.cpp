...
BatchedDataGen::~BatchedDataGen() {
    if (h_key_ref_) delete[] h_key_ref_;
    if (h_index_ref_) delete[] h_index_ref_;
    if (h_batch_buffer_) delete[] h_batch_buffer_;
    if (temp_buffer_) delete[] temp_buffer_;
}
...
uint32_t* BatchedDataGen::getSingleBatchPointer(uint32_t num_keys,
    ...
    auto begin_index =
            (num_keys > num_existing) ? (num_keys - num_existing) : 0;
    ...
    std::shuffle(h_batch_buffer_ + num_keys,
                 h_batch_buffer_ + num_keys + num_queries, rng);
    return h_batch_buffer_;
}

uint32_t BatchedDataGen::get_edge_index() { return edge_index_; }

void BatchedDataGen::set_edge_index(uint32_t new_edge_index) {
    if (new_edge_index < num_ref_) edge_index_ = new_edge_index;
}
...
uint32_t* BatchedDataGen::next_batch(float a_insert,
    ...
    // choosing the first num_search_exist_ from the beginning of the
    // references: code 3 for search queries
