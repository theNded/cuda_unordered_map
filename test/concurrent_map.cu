 ...
 *
 * findEmptyPerWarpLicensed under the Apache License, Version 2.0 (the "License");
...
int main(int argc, char** argv) {
    ...
    GpuHashTable<KeyT, ValueT, HashFunc> hash_table(num_keys, num_buckets, 0, seed);
    ...
    for (int i = 0; i < num_queries; i++) {
        if (SEARCH_NOT_FOUND != h_result[i]) {
            printf("### wrong result at index %d: [%d] -> %d, but should be "
                   "%d\n",
                   i, h_query[i], h_result[i], SEARCH_NOT_FOUND);
            search_success = false;
        }
    }
    if (search_success) {
