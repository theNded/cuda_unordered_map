 ...
 */
template <typename T>
MemoryAlloc<T>::MemoryAlloc(int max_capacity) {
...
template <typename T>
std::vector<int> MemoryAlloc<T>::DownloadHeap() {
    std::vector<int> ret;
...
template <typename T>
std::vector<T> MemoryAlloc<T>::DownloadValue() {
    std::vector<T> ret;
    ret.resize(max_capacity_);
...
template <typename T>
int MemoryAlloc<T>::heap_counter() {
    int heap_counter;
