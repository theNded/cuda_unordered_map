...
public:
    ...
    SlabIterator(SlabHashContext<KeyT, KeyT, SlabHashTypeT::ConcurrentSet>&
        ...
        : slab_hash_(slab_hash),
          ...
          cur_bucket_(0),
          cur_slab_address_(*slab_hash.getPointerFromBucket(
                  0, SlabHashT::NEXT_PTR_LANE)) {}
    ...
    // this function is being run by only one thread, so it is wrong to assume
    // all threads within a warp have access to the caller's iterator state
    __device__ __forceinline__ bool next() {
        ...
        cur_slab_address_ = *slab_hash_.getPointerFromSlab(
                cur_slab_address_, SlabHashT::NEXT_PTR_LANE);
