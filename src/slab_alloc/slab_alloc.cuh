...
public:
    ...
    __device__ __forceinline__ void createMemBlockIndex(
    ...
    __device__ __forceinline__ void updateMemBlockIndex(
            uint32_t global_warp_id) {
        num_attempts_++;
        super_block_index_++;
    ...
    __device__ __forceinline__ uint32_t warpAllocateBulk(uint32_t& lane_id,
        ...
        while (allocated_result == 0xFFFFFFFF) {
            empty_lane =
                    32 -
                    (__ffs(__brev(
                            ~resident_bitmap_)));  // reversing the order of
  ...
  */
    __device__ __forceinline__ void freeUntouched(SlabAllocAddressT ptr) {
        atomicAnd(d_super_blocks_ +
                          getSuperBlockIndex(ptr) * SUPER_BLOCK_SIZE_ +
...
struct __align__(32) ConcurrentSlab {
